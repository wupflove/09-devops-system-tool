...
——配置的最小粒度应对应唯一标识，其内容应不限于KV，Json, XML等格式，用户应能自由配置
...
——配置文件能区分版本，不同版本之间支持快速回滚，以降低配置发错的风险。

——配置管理中心出现不可用时，不影响线上应用的正常运行。

##### 可以包含以下高级功能：

——提供相应程序接口供应用程序实现读、写配置的功能。

——提供相应程序接口供应用程序实现配置订阅的功能，如配置变更则程序可在短时间内做出响应。

——应用程序应可以基于多类语言的SDK来操作配置，包括但不限于java, go, node, c++

——配置发布时应支持灰度发布，如根据某类对象的IP或某类对象的Label

——提供配置订阅监控，可查看配置被订阅的所有对象

——配置管理能集成加密工具以保证配置传输的安全性
